...
# Function #1: fasta_to_lazyframe
def fasta_to_lazyframe(fasta):
	"""
	Converts a FASTA file to a polars LazyFrame object. 

	Args:
		fasta (str): Path to the FASTA file.

	Returns:
		pl.LazyFrame: A LazyFrame with 'header' and 'sequence' columns.
	"""
	...
	dl = pl.LazyFrame({'headers': headers, 'sequences': sequences})
...
# Function #3: form_lazyframe
def form_lazyframe(dl, pattern = "N+"):
	"""
	Forms a polars LazyFrame object from a FASTA file and performs necessary operation to get into BED format.

	Args:
		dl (pl.LazyFrame): A LazyFrame with 'header' and 'sequence' columns created by the fasta_to_lazyframe function.
	...
	Returns:
		pl.LazyFrame: A LazyFrame with 'header', 'start', and 'end' columns in BED format, with 'start' and 'end'
		being generated by the find_subsequence_range function.
	"""

	# Take the input LazyFrame and perform the necessary operations to get it into BED format
	dl = (
		dl
		.with_columns(
...
def main():
	...
	# Convert the FASTA file to a LazyFrame
	dl = fasta_to_lazyframe(fasta)

	# Form the LazyFrame into a BED formated LazyFrame
	dl = form_lazyframe(dl)

	# Set the BED file path
	bed = args.bed

	# Save the LazyFrame to a BED file
	dl.sink_csv(
		path = bed,
